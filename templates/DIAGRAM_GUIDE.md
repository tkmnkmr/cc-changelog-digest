...
06 は `.cols3` のテキストリストのみで構成する。なお report.html の `.cols3` は
✨新規 / 🔧更新 / 🐛修正 / 🗑️廃止・利用不可 の**4列**）。card.html や他セクションで
...
> Claude Code v9.9.9、地味だけど効くやつが来た。長時間セッション後の Bash 権限確認が減って、read-only な git コマンドが素通りするように。ついでに新モデルも追加でコード生成がさらに速く。破壊的変更が1件あるので移行ガイドだけ要チェック。 #ClaudeCode
